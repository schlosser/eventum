...
def new_post():
    ...
    return render_template('blog/edit_post.html', user=g.user, form=form, images=images)

@blog.route('/blog/posts/edit/<post_id>', methods=['GET', 'POST'])
@requires_privilege('edit')
def edit_post(post_id):
    ...
    return render_template('blog/edit_post.html', user=g.user, form=form, post=post, images=images)

@blog.route('/blog/posts/delete/<post_id>', methods=['POST'])
def delete(post_id):
    object_id = ObjectId(post_id)
    if BlogPost.objects(id=object_id).count() == 1:
        post = BlogPost.objects().with_id(object_id)
...
@blog.route('/blog/posts/publish/<post_id>', methods=['POST'])
@requires_privilege('publish')
def publish_event(post_id):
    return set_published_status(post_id, True)


@blog.route('/blog/posts/unpublish/<post_id>', methods=['POST'])
@requires_privilege('publish')
def unpublish_event(post_id):
    return set_published_status(post_id, False)

@blog.route('/blog/posts/edit/epiceditor/themes/<folder>/<path>')
@blog.route('/blog/posts/epiceditor/themes/<folder>/<path>')
@login_required
